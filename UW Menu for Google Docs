...
//  var selection = SpreadsheetApp.getActiveSheet().getActiveCell().getValue();
  ...
  DocumentApp.getUi().showModalDialog(userInterface, 'Batch List');
}
...
//  var selection = SpreadsheetApp.getActiveSheet().getActiveCell().getValue();
  ...
  var userInterface = HtmlService.createHtmlOutput(html);
  DocumentApp.getUi().showModalDialog(userInterface, 'Upload Metadata');
}
...
function toManual(){
  ...
  var userInterface = HtmlService.createHtmlOutput(html);
  DocumentApp.getUi().showModalDialog(userInterface, 'User Manual');
}
...
function toErrorLog(){
  ...
  var userInterface = HtmlService.createHtmlOutput(html);
  DocumentApp.getUi().showModalDialog(userInterface, 'Error Log');
}
